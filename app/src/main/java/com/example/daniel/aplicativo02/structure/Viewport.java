...
public class Viewport {
    ...
    public Viewport(Point sceneDimensions, Point screenDimensions, ScalingMode scalingMode) {
        ...
        mOffsetFromOrigin.x =
                (int) ((screenDimensions.x - (sceneDimensions.x * mScalingFactor.x)) / 2);
        mOffsetFromOrigin.y =
                (int) ((screenDimensions.y - (sceneDimensions.y * mScalingFactor.y)) / 2);
        mDrawingArea.left = mOffsetFromOrigin.x;
        mDrawingArea.top = mOffsetFromOrigin.y;
        mDrawingArea.right = (int) ((sceneDimensions.x * mScalingFactor.x)
                + mOffsetFromOrigin.x);
        mDrawingArea.bottom = (int) ((sceneDimensions.y * mScalingFactor.y)
                + mOffsetFromOrigin.y);
    }
